...
#
# Write-Host is avoided because it cannot be suppressed, captured or redirected; Write-Information
# with -InformationAction Continue reaches the console the same way while staying redirectable.
# Colour is applied with ANSI escapes rather than -ForegroundColor, which Write-Information lacks.
$script:AnsiColours = @{
	Red    = "$([char]27)[31m"
	Green  = "$([char]27)[32m"
	Yellow = "$([char]27)[33m"
...
function Write-Message {
	param(
		...
		[ValidateSet('Red', 'Green', 'Yellow', 'Cyan')]
		[string]$Colour
	)

	$text = $Message
	if ($Colour -and $Host.UI.SupportsVirtualTerminal) {
		$text = "$($script:AnsiColours[$Colour])$Message$([char]27)[0m"
	}
